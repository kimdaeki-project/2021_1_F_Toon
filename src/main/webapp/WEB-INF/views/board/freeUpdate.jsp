...
<c:import url="../fragments/header.jsp"></c:import>
 ...
 <h2>
			<p>
				<span
					style="border-radius: 15px 15px 15px 15px; border: 3px solid #6C7AE0; padding: 0.5em 0.6em; color: white; background-color: #6C7AE0;">자유게시판/수정</span>
			</p>
		</h2><br>
  <form id="frm" action="./freeUpdate" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
      ...
      <input type="text" class="form-control" id="freeTitle" name="freeTitle" value="${vo.freeTitle}">
    </div>
    
    <div class="form-group">
				<label for="freeContents">&nbsp;내용</label>
				<textarea class="form-control myCheck" rows="5" id="freeContents"
					name="freeContents">${vo.freeContents}</textarea>
			</div>
    ...
    <c:forEach items="${vo.freefiles}" var="file">
    	<div>
    		...
    		&emsp;<span class="fileDelete" title="${file.fileNum}">파일삭제</span>
    	</div><br>
    </c:forEach><br>
    
    &nbsp;&nbsp;<input type="button" id="add" value="파일추가" class="btn btn-primary">
			
	<div id="files" title="${vo.freefiles.size()}"></div><br>
